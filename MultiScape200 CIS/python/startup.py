...
# Setup Additional OFE Temperature Sensors
imager.ofe_temps = 16
imager.ofe_tlm_info.extend([
                            {'Name':'T_OFE+X270'      , 'Unit':'`C', 'Used':True  , 'Range':{'Min':  -20, 'Max':   60}},
                            {'Name':'T_OFE+Y270'      , 'Unit':'`C', 'Used':True  , 'Range':{'Min':  -20, 'Max':   60}},
                            {'Name':'T_OFE-X270'      , 'Unit':'`C', 'Used':True  , 'Range':{'Min':  -20, 'Max':   60}},
                            {'Name':'T_OFE-Y270'      , 'Unit':'`C', 'Used':True  , 'Range':{'Min':  -20, 'Max':   60}}])
...
def PrintOfeTlm():
    ...
    Get and Print FEE Telemetry
    """
    imager.GetOfeTelemetry()
    WaitCmdDone(1)
    tlm = imager.ReqOfeTelemetry()    
    print ("OFE Telemetry:")
    for i in range(len(tlm)):
        tlminfo = imager.ofe_tlm_info[i]
        print(f"{i:>2}  {tlminfo['Name']:12} : {tlm[i]:>3} {tlminfo['Unit']}")
...
def SetAllDefaultImagingParameters():
    ...
    for i in range (imager.bands):
        ...
        WaitCmdDone()
    imager.SetDefaultImagingParameter(0x33,0,3436) # Calibrated Value
    WaitCmdDone()
    imager.SetDefaultImagingParameter(0x33,1,4672) # Calibrated Value 
    WaitCmdDone()
    imager.SetDefaultImagingParameter(0x33,2,4300) # Calibrated Value
    WaitCmdDone()    
    imager.SetDefaultImagingParameter(0x33,3,3888) # Calibrated Value
    WaitCmdDone()
    imager.SetDefaultImagingParameter(0x33,4,2676) # Calibrated Value
    WaitCmdDone()
    imager.SetDefaultImagingParameter(0x33,5,2484) # Calibrated Value
    WaitCmdDone()
    imager.SetDefaultImagingParameter(0x33,6,2072) # Calibrated Value
    WaitCmdDone()
    imager.SetDefaultImagingParameter(0x33,7,3172) # Calibrated Value
    WaitCmdDone()
