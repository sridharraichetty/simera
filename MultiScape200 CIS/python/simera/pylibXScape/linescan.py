...
IMAGING_PARAMID_LINESCAN_LINE_BINNING = 0x3B

IMAGING_PARAM_BINNING_FACTORS = [0, 2, 4]

class LineScan(xscape.xScape):
    """
    LineScan class

    Inherits from the xScape class and extends it for line scan specific commands and requests
    """

    def __init__(self, EGSE = None, I2Caddr = None): # override the constructor
        """
        LineScan Constructor
        ...
        self.imaging_param_parser.update( {
                ...
                IMAGING_PARAMID_LINESCAN_LINE_BINNING       : lambda *x : self._parseImagingParameter_LinescanLineBinning(*x)
            })
        ...
        self.imaging_param_req_handlers.update( {
                ...
                IMAGING_PARAMID_LINESCAN_LINE_BINNING       : lambda  : self._handleImagingParameterReq_LinescanLineBinning()
            })
    ...
    def _parseImagingParameter_LinescanLineBinning(self, *Values):
        """
        Parse command payload for the binning reduction factor for the specified band
    
        _parseImagingParameter_LinescanLineBinning(Band, Redfactor)

        Mandatory Arguments:
            Band      (uint8) - The Band Number
            Redfactor (uint8) - Reduction Factor (0 = No binning (1x1), 2 = 2x2 binning, 4 = 4x4 binning)
        """
        try:
            Band = int(Values[0])
        except ValueError as e:
        ...
        try:
            Redfactor = int(Values[1])
        except ValueError as e:
            raise exceptions.InputError(f'Binning reduction factor parameter must be an integer, but "{Values[0]}" was supplied.\n{e}')
        if Redfactor not in IMAGING_PARAM_BINNING_FACTORS:
            raise exceptions.InputError(f'Binning reduction factor parameter must be one of {IMAGING_PARAM_BINNING_FACTORS}, but "{Redfactor}" was supplied.')

        return [Band, Redfactor]                 
    ...
    def _handleImagingParameterReq_LinescanLineBinning(self):
        """
        Return the Band Binning Reduction Factor    
        """      
        req_id = 0x89
        req_length = self.bands
